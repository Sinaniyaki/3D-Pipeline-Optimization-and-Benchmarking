...
/* Fixed: add tag names to the struct definitions */
...
Node_float* construct_subtree_float(float *pa, uint32_t *pidx, int8_t no_dims, uint32_t start_idx, uint32_t n, uint32_t bsp, float *bbox)
{
    ...
    else
    {
        ...
        root->left_child = (Node_float *)construct_subtree_float(pa, pidx, no_dims, start_idx, n_lo, bsp, bbox);
        bbox[2 * cut_dim + 1] = hv;

        bbox[2 * cut_dim] = cut_val;
        root->right_child = (Node_float *)construct_subtree_float(pa, pidx, no_dims, start_idx + n_lo, n - n_lo, bsp, bbox);
        bbox[2 * cut_dim] = lv;
    }
    return root;
}
...
Tree_float* construct_tree_float(float *pa, int8_t no_dims, uint32_t n, uint32_t bsp)
{
    ...
    tree->root = (Node_float *)construct_subtree_float(pa, pidx, no_dims, 0, n, bsp, bbox);
...
void delete_subtree_float(Node_float *root)
{
    if (root->cut_dim != -1)
    {
        delete_subtree_float(root->left_child);
        delete_subtree_float(root->right_child);
    }
    free(root);
}
...
void delete_tree_float(Tree_float *tree)
{
    delete_subtree_float(tree->root);
    free(tree->bbox);
    free(tree->pidx);
    free(tree);
}
...
void print_tree_float(Node_float *root, int level)
{
    ...
    if (root->cut_dim != -1)
        print_tree_float(root->left_child, level + 1);
    if (root->cut_dim != -1)
        print_tree_float(root->right_child, level + 1);
}
...
Params:
    ...
    n : number of data points
    point_coord : query point
    mask : boolean array (True means invalid)
    closest_idx : index of closest data point found (return)
    closest_dist : distance to closest point (return)
************************************************/
void search_leaf_float_mask(float *restrict pa, uint32_t *restrict pidx, int8_t no_dims, uint32_t start_idx, uint32_t n, float *restrict point_coord,
...
Params:
    ...
    no_dims : number of dimensions
    point_coord : query point
    min_dist : minimum distance to nearest neighbor
    mask : boolean array (True means invalid)
    closest_idx : index of closest data point found (return)
    closest_dist : distance to closest point (return)
************************************************/
                      ...
                      uint32_t *closest_idx, float *closest_dist)
{
    ...
    if (new_offset < 0)
    {
        ...
        {
            search_splitnode_float(root->left_child, pa, pidx, no_dims, point_coord, dist_left, k, distance_upper_bound, eps_fac, mask, closest_idx, closest_dist);
        }
        ...
        {
            search_splitnode_float(root->right_child, pa, pidx, no_dims, point_coord, dist_right, k, distance_upper_bound, eps_fac, mask, closest_idx, closest_dist);
        }
    }
    else
    {
...
/************************************************
Search for nearest neighbor for a set of query points
Params:
    tree : Tree struct of kd tree
    pa : data points
    ...
    mask : boolean array (True means invalid)
    closest_idx : indices of closest points (return)
    closest_dist : distances to closest points (return)
************************************************/
void search_tree_float(Tree_float *tree, float *pa, float *point_coords,
                 uint32_t num_points, uint32_t k, float distance_upper_bound,
                 float eps, uint8_t *mask, uint32_t *closest_idxs, float *closest_dists)
...
/************************************************
Insert point into priority queue (double version)
************************************************/
void insert_point_double(uint32_t *closest_idx, double *closest_dist, uint32_t pidx, double cur_dist, uint32_t k)
{
    int i;
...
/************************************************
Get the bounding box of a set of points (double version)
************************************************/
void get_bounding_box_double(double *pa, uint32_t *pidx, int8_t no_dims, uint32_t n, double *bbox)
{
    double cur;
...
/************************************************
Partition a range of data points (double version)
************************************************/
...
/************************************************
Construct a sub tree over a range of data points (double version)
************************************************/
Node_double* construct_subtree_double(double *pa, uint32_t *pidx, int8_t no_dims, uint32_t start_idx, uint32_t n, uint32_t bsp, double *bbox)
{
    int is_leaf = (n <= bsp);
    ...
    else
    {
        ...
        bbox[2 * cut_dim + 1] = cut_val;
        root->left_child = construct_subtree_double(pa, pidx, no_dims, start_idx, n_lo, bsp, bbox);
        bbox[2 * cut_dim + 1] = hv;

        bbox[2 * cut_dim] = cut_val;
        root->right_child = construct_subtree_double(pa, pidx, no_dims, start_idx + n_lo, n - n_lo, bsp, bbox);
        bbox[2 * cut_dim] = lv;
    }
    return root;
}
...
/************************************************
Construct a tree over data points (double version)
************************************************/
Tree_double* construct_tree_double(double *pa, int8_t no_dims, uint32_t n, uint32_t bsp)
{
    ...
    tree->root = construct_subtree_double(pa, pidx, no_dims, 0, n, bsp, bbox);
...
/************************************************
Create a tree node (double version)
************************************************/
Node_double* create_node_double(uint32_t start_idx, uint32_t n, int is_leaf)
{
...
/************************************************
Delete subtree (double version)
************************************************/
void delete_subtree_double(Node_double *root)
{
    if (root->cut_dim != -1)
    {
        delete_subtree_double(root->left_child);
        delete_subtree_double(root->right_child);
    }
    free(root);
}

/************************************************
Delete tree (double version)
************************************************/
void delete_tree_double(Tree_double *tree)
{
    delete_subtree_double(tree->root);
    free(tree->bbox);
    free(tree->pidx);
    free(tree);
}
...
void print_tree_double(Node_double *root, int level)
{
    ...
    printf("(cut_val: %f, cut_dim: %i)\n", root->cut_val, root->cut_dim);
    if (root->cut_dim != -1)
        print_tree_double(root->left_child, level + 1);
    if (root->cut_dim != -1)
        print_tree_double(root->right_child, level + 1);
}

/************************************************
Calculate squared cartesian distance between points (double version)
************************************************/
double calc_dist_double(double *point1_coord, double *point2_coord, int8_t no_dims)
{
...
/************************************************
Get squared distance from point to cube in specified dimension (double version)
************************************************/
double get_cube_offset_double(int8_t dim, double *point_coord, double *bbox)
{
...
/************************************************
Get minimum squared distance between point and cube (double version)
************************************************/
double get_min_dist_double(double *point_coord, int8_t no_dims, double *bbox)
{
...
/************************************************
Search a leaf node for closest point (double version)
************************************************/
void search_leaf_double(double *restrict pa, uint32_t *restrict pidx, int8_t no_dims, uint32_t start_idx, uint32_t n, double *restrict point_coord,
                 uint32_t k, uint32_t *restrict closest_idx, double *restrict closest_dist)
{
...
/************************************************
Search a leaf node for closest point with mask (double version)
************************************************/
void search_leaf_double_mask(double *restrict pa, uint32_t *restrict pidx, int8_t no_dims, uint32_t start_idx, uint32_t n, double *restrict point_coord,
                               uint32_t k, uint8_t *restrict mask, uint32_t *restrict closest_idx, double *restrict closest_dist)
{
...
/************************************************
Search subtree for nearest neighbor (double version)
************************************************/
void search_splitnode_double(Node_double *root, double *pa, uint32_t *pidx, int8_t no_dims, double *point_coord, 
                      ...
                      uint32_t *closest_idx, double *closest_dist)
{
    ...
    if (new_offset < 0)
    {
        ...
        if (dist_left < closest_dist[k - 1] * eps_fac)
        {
            search_splitnode_double(root->left_child, pa, pidx, no_dims, point_coord, dist_left, k, distance_upper_bound, eps_fac, mask, closest_idx, closest_dist);
        }
        ...
        if (dist_right < closest_dist[k - 1] * eps_fac)
        {
            search_splitnode_double(root->right_child, pa, pidx, no_dims, point_coord, dist_right, k, distance_upper_bound, eps_fac, mask, closest_idx, closest_dist);
        }
    }
    else
    {
...
/************************************************
Search for nearest neighbor for a set of query points (double version)
************************************************/
void search_tree_double(Tree_double *tree, double *pa, double *point_coords,
                 uint32_t num_points, uint32_t k, double distance_upper_bound,
                 double eps, uint8_t *mask, uint32_t *closest_idxs, double *closest_dists)
