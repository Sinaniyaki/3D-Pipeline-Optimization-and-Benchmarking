...
if __name__ == '__main__':
    ...
    for it, data in enumerate(tqdm(test_loader)):
        ...
        try:
            ...
            mesh_times.append(time_dict['mesh'])

            if torch.cuda.is_available():
                time_dict['gpu_mem_MB'] = torch.cuda.max_memory_allocated() / 1024**2
                print(f"[{modelname}] Inference time: {time_dict['mesh']:.4f}s | GPU memory: {time_dict['gpu_mem_MB']:.2f} MB")
    ...
    avg_time_per_shape = np.mean(mesh_times)
    print(f"\nAverage inference time per shape: {avg_time_per_shape:.4f} seconds")
