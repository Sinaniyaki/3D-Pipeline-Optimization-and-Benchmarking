...
if __name__ == '__main__':
    ...
    # Set mesh extraction to low resolution for fast visualization during training
    ...
    trainer = config.get_trainer(model, optimizer, cfg, device=device, generator=generator)
    ...
    while True:
        ...
        for batch in train_loader:
            ...
            # STOP if max it reached
            if it >= 30000:
                logger_py.info('Reached 30,000 iterations. Stopping training.')
            ...
            # Measure time and log to TensorBoard
            iter_time = time.time() - start_iter_time
            logger.add_scalar('perf/iter_time_sec', iter_time, it)

            # Log peak GPU memory usage
            if torch.cuda.is_available():
                gpu_mem = torch.cuda.max_memory_allocated() / 1024 ** 2
                logger.add_scalar('perf/gpu_mem_MB', gpu_mem, it)
            ...
            if print_every > 0 and (it % print_every) == 0:
                logger_py.info('[Epoch %02d] it=%03d, loss=%.4f, time=%.4f, iter_time=%.4f s, GPU mem=%.2f MB'
                               % (epoch_it, it, loss, time.time() - t0b, iter_time, gpu_mem if torch.cuda.is_available() else 0))
                t0b = time.time()
            ...
            # Auto-exit if time limit reached
